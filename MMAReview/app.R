...
# Define UI
ui <- dashboardPage(
  dashboardHeader(title = "MMA Sim Review"),
  ...
  observeEvent(input$process_btn, {
    ...
    sim_data <- read.csv(sim_file, stringsAsFactors = FALSE) %>% 
      mutate(
        ...
        Fighter1 = str_replace(Fighter1, "\\s*\\(\\d+\\)", ""),
        Fighter2 = str_replace(Fighter2, "\\s*\\(\\d+\\)", ""),
        Fighter3 = str_replace(Fighter3, "\\s*\\(\\d+\\)", ""),
        Fighter4 = str_replace(Fighter4, "\\s*\\(\\d+\\)", ""),
        Fighter5 = str_replace(Fighter5, "\\s*\\(\\d+\\)", ""),
        Fighter6 = str_replace(Fighter6, "\\s*\\(\\d+\\)", "")
      ) %>%
      rowwise() %>%
      mutate(
        # Sort Fighters alphabetically
        Fighters_sorted = list(sort(c(Fighter1, Fighter2, Fighter3, Fighter4, Fighter5, Fighter6))),
        
        # Assign sorted Fighters to columns
        Fighter1 = Fighters_sorted[1],
        Fighter2 = Fighters_sorted[2],
        Fighter3 = Fighters_sorted[3],
        Fighter4 = Fighters_sorted[4],
        Fighter5 = Fighters_sorted[5],
        Fighter6 = Fighters_sorted[6]
      ) %>%
      ungroup() %>%
      # Remove temporary column
      select(-Fighters_sorted)
    ...
    scored_sims <- sim_data %>%
      mutate(
        Fighter1Score = score_lookup[Fighter1],
        Fighter2Score = score_lookup[Fighter2],
        Fighter3Score = score_lookup[Fighter3],
        Fighter4Score = score_lookup[Fighter4],
        Fighter5Score = score_lookup[Fighter5],
        Fighter6Score = score_lookup[Fighter6],
        
        TotalScore = Fighter1Score + Fighter2Score + Fighter3Score + 
          Fighter4Score + Fighter5Score + Fighter6Score
      ) %>%
    ...
    # Parse Fighters from Lineup
    contest_formatted$Fighter1 <- NA
    contest_formatted$Fighter2 <- NA
    contest_formatted$Fighter3 <- NA
    contest_formatted$Fighter4 <- NA
    contest_formatted$Fighter5 <- NA
    contest_formatted$Fighter6 <- NA
    ...
    for (i in 1:nrow(contest_data)) {
      ...
      # Split at " D " markers
      Fighter_split <- unlist(str_split(current_lineup, "F\\s+"))
      
      # Remove the first empty element
      Fighters <- str_trim(Fighter_split[Fighter_split != ""])
      
      # Sort Fighters to match simulation format
      Fighters_sorted <- sort(Fighters)
      
      # Assign to the correct row in contest_formatted
      if (length(Fighters_sorted) >= 1) contest_formatted$Fighter1[i] <- Fighters_sorted[1]
      if (length(Fighters_sorted) >= 2) contest_formatted$Fighter2[i] <- Fighters_sorted[2]
      if (length(Fighters_sorted) >= 3) contest_formatted$Fighter3[i] <- Fighters_sorted[3]
      if (length(Fighters_sorted) >= 4) contest_formatted$Fighter4[i] <- Fighters_sorted[4]
      if (length(Fighters_sorted) >= 5) contest_formatted$Fighter5[i] <- Fighters_sorted[5]
      if (length(Fighters_sorted) >= 6) contest_formatted$Fighter6[i] <- Fighters_sorted[6]
    }
    ...
    sim_with_keys <- scored_sims %>%
      ...
      select(lineup_key, TotalScore, Top1Count, Rank, Fighter1, 
             Fighter2, Fighter3, Fighter4, Fighter5, Fighter6,
    ...
    contest_with_keys <- contest_formatted %>%
      ...
      select(lineup_key, Rank, EntryName, Points, Fighter1, 
             Fighter2, Fighter3, Fighter4, Fighter5, Fighter6) %>% 
      filter(Points > 0)
  ...
  output$contest_lineup_table <- renderDT({
    ...
    data_store$matches %>%
      ...
      select(
        Rank = Rank_contest,
        User = EntryName,
        Fighter1 = Fighter1_contest, 
        Fighter2 = Fighter2_contest, 
        Fighter3 = Fighter3_contest, 
        Fighter4 = Fighter4_contest, 
        Fighter5 = Fighter5_contest,
        Fighter6 = Fighter6_contest,
        Points, 

...
# Define UI
ui <- dashboardPage(
  dashboardHeader(title = "NASCAR Sim Review"),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$process_btn, {
    ...
    sim_data <- read.csv(sim_file, stringsAsFactors = FALSE) %>% 
      mutate(
        # Create ranking based on Top1Count
    ...
    sim_with_keys <- scored_sims %>%
      ...
      select(lineup_key, TotalScore, Top1Count, Rank, Driver1, 
    ...
    contest_with_keys <- contest_formatted %>%
      ...
      select(lineup_key, Rank, EntryName, Points, Driver1, 
             Driver2, Driver3, Driver4, Driver5, Driver6) %>% 
      filter(Points > 0)
